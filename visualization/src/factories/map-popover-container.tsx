...
function getLayerHoverProp({
    ...
    if (hoverInfo.layer?.id === EDITOR_LAYER_ID) {
        ...
        // filter out roads whose type does not match the road type selection
        if (selectedRoadTypes.length > 0 && dataId === "road_network") {
            data = data.filter(dataRow => selectedRoadTypes.indexOf(dataRow.valueAt(roadTypeFieldIndex)) !== -1);
        }
...
const MapPopoverContainer = ({
    ...
    function onToggleRoadTypeFilter(roadType: string) {
        ...
        let updatedRoadTypes: React.SetStateAction<any[]>;
        ...
        setSelectedRoadTypes(updatedRoadTypes);
    }

    useEffect(() => {
        // update when the road type selection changes
    }, [selectedRoadTypes])
    ...
    let unselectedLayerId: number;
    if (selectedFeature) {
        selectedFilterIdx = state.visState.filters.findIndex((f) => f.value.id === selectedFeature.id);
        unselectedLayerIdx = state.visState.layers.findIndex((l) => l.id !== state.visState.filters[0].dataId[0]);
        selectedLayerIdx = state.visState.layers.findIndex((l) => l.id === state.visState.filters[0].dataId[0]);
    ...
    useEffect(() => {
        // Effect to update the component when customToggle changes
    }, [customToggle]);
