...
def run_xai_blackbox(
    ...
    file_pattern = f"results_simulation_{day_type.strip().lower()}_timeslot_{time_slot}_*.zip"
    xai_baseline.model_output_file_path = find_latest_file(derive_output_path(simulation_id), file_pattern=file_pattern)
    ...
    impact_output_path = (
        derive_output_path(simulation_id) / f"{xai_attribute}_{day_type}_{time_slot}_{XAI_IMPACT_FILENAME}"
    )
    ...
    difference_output_path = (
        derive_output_path(simulation_id) / f"{xai_attribute}_{day_type}_{time_slot}_{XAI_DIFFERENCE_FILENAME}"
    )
...
def run_xai_simulations(xai_records: list[XAIRecord], simulation_id: SimulationID, output_path: Path) -> None:
    ...
    for i, xai_record in enumerate(xai_records):
        ...
        if xai_record.model_output_path.exists() and list(xai_record.model_output_path.glob("*.zip")):
            logging.info(f"XAI simulation for road ID {xai_record.road_id} already exists")
        else:
...
def run_model(
    input_file_path: Path, output_path: Path, create_trajectories: bool = False, flood_model_input: Path = None
) -> int:
    """
    ...
    if flood_model_input:
        run_cmd += ["--flood-depth-map", flood_model_input.absolute()]
