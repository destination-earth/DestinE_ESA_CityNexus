...
def load_geojson(file_path: Path, type_loader: Callable[[IO[bytes]], T]) -> T:
    """
    Loads a geojson file from the local data system and converts it into a usable type.
    :param file_path: The local file path of the geojson file to load.
    :param type_loader: A function that converts the JSON files to a usable type.
    :return: The converted geojson file. The type depends on the return type of the type_loader.
    """
...
def load_as_dict(file_bytes: [IO[bytes]]) -> dict:
    ...
    :param file_bytes: The result JSON file unzipped in memory.
