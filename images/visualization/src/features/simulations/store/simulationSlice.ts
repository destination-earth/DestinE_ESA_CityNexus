...
export const isPredictionNameUsed = createAsyncThunk('simulation/isPredictionNameUsed', async ({name, scenarioId}: {name: string, scenarioId: string}, thunkAPI) => {
    const api = ApiFactory.build(thunkAPI.getState());
    return api.isPredictionNameUsed(name, scenarioId);
});

export const isScenarioNameUsed = createAsyncThunk('simulation/isScenarioNameUsed', async ({name, city}: {name: string, city: string}, thunkAPI) => {
    const api = ApiFactory.build(thunkAPI.getState());
    return api.isScenarioNameUsed(name, city);
});
...
export const createScenario = createAsyncThunk('simulation/createScenario', async (data: any, thunkAPI) => {
    ...
    const changesToSave = state.demo.undoRedo.changes;
    return api.createScenario(data.name, data.project, data.city, {description: data.description, data: changesToSave});
});
...
export const updateScenario = createAsyncThunk('simulation/updateScenario', async (name: string, thunkAPI) => {
    ...
    if (!name) {
        name = state.demo.undoRedo.changes.scenarioName;
    }

    const changesToSave = state.demo.undoRedo.changes;
    const scenarioId = state.demo.undoRedo.changes.scenarioId;
