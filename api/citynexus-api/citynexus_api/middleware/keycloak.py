...
keycloak_config = KeycloakConfiguration(
    ...
    claims=["resource_access", "sub", "roles"],
    reject_on_missing_claim=False,
)

possible_roles = ["immerseon-paid", "immerseon-free", "citynexus-paid", "citynexus-free"]
...
async def map_user(userinfo: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """
    roles = {}
    for item in userinfo["roles"]:
        if item in possible_roles:
            project, paid = item.split("-")
            if not project in roles:
                roles[project] = False
...
async def get_user(request: Request) -> Dict[str, Any]:
    ...
    if "user" in request.scope:
        ...
        return {"user": user_name, "access": user_access, "roles": user_roles}
    else:
        return {"user": None, "access": None, "roles": []}
